...
class WebSocketManager
{
    ...
    {
        if (!isset($this->connections[$connection->id])) return;
    ...
    {
        if (!isset($this->rooms[$roomId])) return 0;
        $count = 0;
        $msg = json_encode($data, JSON_UNESCAPED_UNICODE);
        foreach ($this->rooms[$roomId] as $connId => $true) {
            if ($exclude && $connId === $exclude->id) continue;
            if (isset($this->connections[$connId])) {
                $this->connections[$connId]['connection']->send($msg);
                $count++;
            }
        }
        return $count;
...
// 如果是工作进程模式，启动 Worker
if (isWorkerProcess()) {
	$worker = new Worker('http://0.0.0.0:8000, websocket://0.0.0.0:1234');
	$worker->name = 'FSSPHP-ALL'; // HTTP + WebSocket
	$worker->count = 1;
    $framework = null;
    ...
    $worker->onMessage = function(TcpConnection $connection, WorkermanRequest $req) use (&$framework) {
		...
		if ($connection->protocol === 'Workerman\Protocols\WebSocket') {
			$wsMgr = WebSocketManager::getInstance();
			
			// 首次连接
			if (!isset($connection->ws_handshaked)) {
				$connection->ws_handshaked = true;
				$wsMgr->addConnection($connection);
				$wsMgr->sendToConnection($connection, [
					'type' => 'connected',
			...
			try {
				...
				switch ($msg['type'] ?? '') {
					case 'ping':
						$wsMgr->sendToConnection($connection, ['type' => 'pong']);
						break;
					case 'bind':
						$wsMgr->bindUser($connection, $msg['data']['user_id'] ?? null);
						break;
					case 'join':
						$wsMgr->joinRoom($connection, $msg['data']['room_id'] ?? 'default');
						break;
					case 'leave':
						$wsMgr->leaveRoom($connection, $msg['data']['room_id'] ?? 'default');
						break;
				}
			} catch (Throwable $e) {}
			return;
	...
	$worker->onClose = function(TcpConnection $conn) {
		if ($conn->protocol === 'Workerman\Protocols\WebSocket') {
			WebSocketManager::getInstance()->removeConnection($conn);
		}
	};

	// 错误处理
	$worker->onError = function(TcpConnection $conn, $code, $msg) {
		// 静默错误
	};
